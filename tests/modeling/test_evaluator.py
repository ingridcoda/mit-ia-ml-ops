...
def test_diagnostico_robustez_boa(null_logger):
    ...
    # Diferença de 2.5% (abaixo do limite de 15% definido no código)
...
def test_buscar_melhor_threshold_logica(null_logger):
    ...
    # Probabilidades que separam perfeitamente as classes em 0.5
    y_prob = np.array([0.1, 0.2, 0.8, 0.9])

    threshold, score = evaluator.buscar_melhor_threshold(y_true, y_prob)

    # O threshold deve estar entre o maior valor da classe 0 e menor da classe 1
    assert 0.2 < threshold < 0.8
    assert score == 1.0  # F1 perfeito esperado para este conjunto


# ── TESTES DE INTEGRAÇÃO DO AVALIADOR (MOCKS) ────────────────────────────────

def test_avaliar_fluxo_com_probabilidade(null_logger):
    """Testa o caminho principal: modelo que suporta predict_proba."""
    evaluator = HoldoutEvaluator(null_logger)

    # Mock configurado para ter apenas o método predict_proba
    model = MagicMock(spec=['predict_proba'])
    # Simula retorno de probabilidade [prob_classe_0, prob_classe_1]
    ...
    # Verifica se o threshold otimizado foi incluído nas métricas
...
def test_avaliar_fluxo_sem_probabilidade(null_logger):
    ...
    # Não deve haver threshold otimizado se o modelo não fornece probabilidades
