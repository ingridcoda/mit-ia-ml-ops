...
from sklearn.model_selection import StratifiedKFold  # Mudança para classificação
...
class CVRunner:
    ...
    def executar(self, modelo: Any, X: pd.DataFrame, y: pd.Series) -> list[dict]:
        metricas_folds = []
        # StratifiedKFold garante a distribuição do target em cada fold
        for i_fold, (idx_treino, idx_val) in enumerate(self.cv.split(X, y)):
            ...
            # Tentamos obter probabilidades para o ROC-AUC
