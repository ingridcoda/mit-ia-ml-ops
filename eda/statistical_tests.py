"""statistical_tests.py — Testes de Independência para Crédito."""
import json
...
def run(df, config, dirs, logger):
    logger.info("=== Testes Estatísticos — Qui-Quadrado e Mann-Whitney ===")
    target = config['feature_selection']['target']

    # 1. Qui-Quadrado (Categorias vs Risco)
    ...
    # 2. Mann-Whitney (Valor do Crédito: Good vs Bad)
