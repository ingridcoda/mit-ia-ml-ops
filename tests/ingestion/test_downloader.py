"""
test_downloader.py — Enhanced unit tests for the KaggleDownloader.
"""
import os
...
def downloader_fake(null_logger):
    ...
    with patch("src.ingestion.downloader.load_dotenv"), patch.dict(os.environ, {"KAGGLE_USERNAME": "test_user",
                                                                                "KAGGLE_KEY": "test_key"}):
...
class TestKaggleDownloader:
    ...
    def test_autenticacao_erro_variavel_faltante(self, mock_load_dotenv, null_logger):
        ...
        with patch.dict(os.environ, {"KAGGLE_KEY": "some_key"}, clear=True):
            with pytest.raises(EnvironmentError, match="Configure Kaggle credentials"):
    ...
    def test_force_download_ignora_arquivos(self, mock_api_class, null_logger, tmp_path):
        ...
        with patch("src.ingestion.downloader.load_dotenv"), patch.dict(os.environ,
